...
:class:`TestingFakts` is a real :class:`~fakts_next.fakts.Fakts` — entering it
(``with`` or ``async with``) publishes it on the ``current_fakts_next``
contextvar exactly like production, so code under test that calls
:func:`~fakts_next.fakts.get_current_fakts_next` (or the ``fakt``/``afakt``
...
Build one with :func:`build_testing_fakts`::
    ...
    with build_testing_fakts(aliases={"alpaka": "http://testserver"}):
        ...  # get_current_fakts_next() now resolves, aliases and tokens work
